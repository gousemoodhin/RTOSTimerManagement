/*
  ...
  - Create 3 timers that register handles "function1", "function2" and
  "function3", respectively. Timer 1 gets invoked every 5 seconds and runs
  function1 which prints current time. Timer 2 gets invoked every 3 seconds and
  runs function1 which prints current time. Timer 3 gets invoked only once 10
  seconds after it was created and which prints current time.
*/
...
void print_program_info(void) {
  ...
  fprintf(stdout, "\n\n-> This Program will initialize the timers and creates "
                  "Timer Task as Thread");
  fprintf(stdout, "\n-> It creates 3 Timers");
...
void function1(void *arg) { print_time_msg(1); }

void function2(void *arg) { print_time_msg(2); }

void function3(void *arg) { print_time_msg(3); }
...
int main(void) {
  ...
  timer_obj1 = RTOSTmrCreate(0, 50, RTOS_TMR_PERIODIC, function1, NULL,
                             timer_name[0], &err_val);
  ...
  // Create Timer2.
  // Provide the required arguments in the function call.
  timer_obj2 = RTOSTmrCreate(0, 30, RTOS_TMR_PERIODIC, function2, NULL,
  ...
  // Create Timer3.
  // Provide the required arguments in the function call.
  timer_obj3 = RTOSTmrCreate(100, 0, RTOS_TMR_ONE_SHOT, function3, NULL,
                             timer_name[2], &err_val);
  // Check the return value and determine if it created successfully or not.
  if (err_val != RTOS_ERR_NONE) {
  ...
  while (getchar() != 'q')
    ...
    // Delete Timers
    if (timer_obj1 && (RTOSTmrDel(timer_obj1, &err_val) == RTOS_FALSE ||
                       err_val == RTOS_ERR_TMR_INVALID)) {
      fprintf(stdout, "\n Delete Timer1 failed, Error: %d timer_obj1 = %p\n",
              err_val, timer_obj1);
    }
  if (timer_obj2 && (RTOSTmrDel(timer_obj2, &err_val) == RTOS_FALSE ||
                     err_val == RTOS_ERR_TMR_INVALID)) {
    fprintf(stdout, "\n Delete Timer2 failed, Error: %d timer_obj2 = %p\n",
            err_val, timer_obj2);
  }
  if (timer_obj3 && (RTOSTmrDel(timer_obj3, &err_val) == RTOS_FALSE ||
                     err_val == RTOS_ERR_TMR_INVALID)) {
